...
def m33_tilted_ring_deproj_radius(coords, model=diskmodel, verbose=False):
    """
    Compute deprojected radius for M33 stars using the tilted ring model,
    safely handling radii beyond the model range by holding the edge values.
    Returns:
        ang_dist2, Rinit, incl, pa  (all in degrees; incl and pa have units)
    """

    converge_i = 0.01 * u.degree
    converge_pa = 0.35 * u.degree

    # Zeroth order deprojected radius (degrees)
    Rinit = m33_tilted_ring_deproj_radius_init(coords)

    # --- Prepare robust interpolators (no crash for out-of-bounds) ---
    f_pa = interp1d(
        model['Radius_arcmin'], model['PA_deg'],
        fill_value=(model['PA_deg'][0], model['PA_deg'][-1]), bounds_error=False
    )
    f_incl = interp1d(
        model['Radius_arcmin'], model['i_deg'],
        fill_value=(model['i_deg'][0], model['i_deg'][-1]), bounds_error=False
    )

    # Main iteration: always use bounds_error=False everywhere!
    # --- First pass ---
    r_arcmin = Rinit * 60.
    pa1 = f_pa(r_arcmin) * u.degree
    incl1 = f_incl(r_arcmin) * u.degree
    alpha, beta = major_minor_transform(coords, pa1, centercoords=m33coord)
    ang_dist = np.sqrt(alpha ** 2 + (beta / np.cos(incl1)) ** 2.)

    # --- Next pass ---
    r2_arcmin = ang_dist * 60.
    pa2 = f_pa(r2_arcmin) * u.degree
    incl2 = f_incl(r2_arcmin) * u.degree
    ...
    if len(np.atleast_1d(ang_dist)) > 1:
        for i, (di, dp, c, r) in enumerate(zip(delta_incl, delta_pa, coords, Rinit)):
            if abs(di) > converge_i or abs(dp) > converge_pa:
                conv_ang_dist, conv_incl, conv_pa = m33_tilted_ring_deproj_converge(
                    c, converge_i, converge_pa, f_incl, f_pa, verbose=verbose
                )
        ...
        if abs(delta_incl) > converge_i or abs(delta_pa) > converge_pa:
            conv_ang_dist, conv_incl, conv_pa = m33_tilted_ring_deproj_converge(
                coords, converge_i, converge_pa, f_incl, f_pa, verbose=verbose
            )
    ...
    if verbose:
        print(f"Number of stars converged in one step: {count} / {len(np.atleast_1d(delta_incl))}")

    # --- Final calculation with converged parameters ---
    final_arcmin = ang_dist * 60.
    pa_final = f_pa(final_arcmin) * u.degree
    incl_final = f_incl(final_arcmin) * u.degree
    alpha, beta = major_minor_transform(coords, pa_final, centercoords=m33coord)
    ang_dist2 = np.sqrt(alpha ** 2 + (beta / np.cos(incl_final)) ** 2.)

    return ang_dist2, Rinit, incl_final, pa_final  # all in degrees
